...
local playerCoords, doorCount = false
...
local DoorLoop = function()
    MRP_CLIENT.TriggerServerCallback('mrp_doorlock:getDoorList', {}, function(doorList)
...
RegisterNetEvent('mrp_doorlock:setState')
AddEventHandler('mrp_doorlock:setState', function(sid, doorID, locked, src)
...
RegisterCommand('doorlock', function()
	if closestDoor.id and not MRP_CLIENT.getPlayerMetadata()['isDead'] and not MRP_CLIENT.getPlayerMetadata()['isCuffed'] then
		...
		TriggerServerEvent('mrp_doorlock:updateState', closestDoor.id, locked, src) -- Broadcast new state of the door to everyone
	end
end)
...
--[[RegisterNetEvent('mrp_doorlock:lockpick') -- Set up your own lockpick event here
AddEventHandler('mrp_doorlock:lockpick', function(data)
	local locked = not closestDoor.data.locked
	TriggerServerEvent('mrp_doorlock:updateState', closestDoor.id, locked, nil, true) -- Broadcast new state of the door to everyone
end)]]
...
RegisterNetEvent('mrp_doorlock:newDoorSetup')
AddEventHandler('mrp_doorlock:newDoorSetup', function(args)
	if not args[1] then
	...
	if doorType == 'door' or doorType == 'sliding' or doorType == 'garage' then
		...
		TriggerServerEvent('mrp_doorlock:newDoorCreate', arg.configname, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, false, doorname)
	...
	elseif doorType == 'double' or doorType == 'doublesliding' then
		...
		if arg then doorname = arg.doorname end
		TriggerServerEvent('mrp_doorlock:newDoorCreate', arg.configname, model, heading, coords, jobs, item, doorLocked, maxDistance, slides, garage, true, doorname)
		print('Successfully sent door data to the server')
		arg = nil
	end
end)
...
RegisterNetEvent('mrp_doorlock:newDoorAdded')
AddEventHandler('mrp_doorlock:newDoorAdded', function(newDoor, doorID, locked)
	Config.DoorList[doorID] = newDoor
	UpdateDoors()
	TriggerEvent('mrp_doorlock:setState', GetPlayerServerId(PlayerId()), doorID, locked)
end)
